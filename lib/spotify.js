...
const QUERY_SONGS_ENDPOINT = 'https://api.spotify.com/v1/search?q=overcast&type=playlist'
...
export const getSongs = async (refresh_token) => {
  const { access_token } = await getAccessToken(refresh_token)
  console.log('refresh token', refresh_token)
  console.log('access token', access_token)
  return fetch(QUERY_SONGS_ENDPOINT, {
    headers: {
      Authorization: `Bearer ${access_token}`
    }
  })
